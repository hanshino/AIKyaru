...
class Embeds:
        ...
        if not data:
            embed = utils.create_embed(
                ...
                author={
                    ...
                    "icon_url": f"https://redive.estertion.win/icon/equipment/102312.webp",
                },
                thumbnail=f"{self.bot.config.get(['AssetsURL'])}/static/images/unit/icon_unit_{unit.id}31.png",
            )
    ...
    def profile(self, unit: Unit):
        ...
        if not data:
            ...
            self.logger.error(
                f"Could not find profile for {unit.name}({unit.id})")
            embed = utils.create_embed(
                title="此角色尚無角色簡介",
                color=unit.color,
                author={
                    "name": f"角色簡介　{unit.name}",
                    "icon_url": f"https://redive.estertion.win/icon/item/31000.webp",
                },
                thumbnail=f"{self.bot.config.get(['AssetsURL'])}/static/images/unit/icon_unit_{unit.id}31.png",
            )
            ...
            embed.add_field(
                name="生日", value=f"{data['birth_month']}月{data['birth_day']}日", inline=True)
            ...
            embed.set_image(
                url=f"{self.bot.config.get(['AssetsURL'])}/static/images/unit_big/unit_big_{unit.id}31.jpg")
        ...
        if not data:
            embed = utils.create_embed(
                ...
                author={
                    ...
                    "icon_url": f"https://redive.estertion.win/icon/item/99002.webp",
                },
                thumbnail=f"{self.bot.config.get(['AssetsURL'])}/static/images/unit/icon_unit_{unit.id}31.png",
            )
            ...
            embed = utils.create_embed(
                ...
                footer={
                    "text": f"括號內為專用裝備滿等時(Lv.{self.bot.gameData.tw.max_enhance_lv})之數值"},
                thumbnail=f"https://redive.estertion.win/equipment/{data['equipment_id']}.webp",
            )
    ...
    def skill(self, unit: Unit):
        ...
        if not data:
            # should not trigger this
            self.logger.error(
                f"Could not find skills for {unit.name}({unit.id})")
            embed = utils.create_embed(
                title="此角色尚無技能資訊",
                color=unit.color,
                author={
                    "name": f"技能資訊　{unit.name}",
                    "icon_url": f"https://redive.estertion.win/icon/skill/2022.webp",
                },
                thumbnail=f"{self.bot.config.get(['AssetsURL'])}/static/images/unit/icon_unit_{unit.id}31.png",
            )
    ...
    def atk_pattern(self, unit: Unit):
        ...
        if not data:
            # should not trigger this
            self.logger.error(
                f"Could not find attack pattern for {unit.name}({unit.id})")
            embed = utils.create_embed(
                title="此角色尚無攻擊模式",
                color=unit.color,
                author={
                    "name": f"攻擊模式　{unit.name}",
                    "icon_url": f"https://redive.estertion.win/icon/equipment/101011.webp",
                },
                thumbnail=f"{self.bot.config.get(['AssetsURL'])}/static/images/unit/icon_unit_{unit.id}31.png",
            )
        else:
